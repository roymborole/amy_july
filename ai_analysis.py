...
def get_analysis_report(raw_data, company_name):
    ...
    try:
        ...
        financial_table += f"""
        <tr><td><span data-tooltip="The total market value of a company's outstanding shares."><strong>Market Cap</strong></span></td><td>{format_large_number(summarized_data['market_cap'])}</td></tr>
        <tr><td><span data-tooltip="The total amount of income generated by the sale of goods or services related to the company's primary operations."><strong>Total Revenue</strong></span></td><td>{summarized_data['Total Revenue']}</td></tr>
        <tr><td><span data-tooltip="Revenue generated from a company's core business operations."><strong>Operating Revenue</strong></span></td><td>{summarized_data['Operating Revenue']}</td></tr>
        <tr><td><span data-tooltip="Earnings Per Share: The portion of a company's profit allocated to each outstanding share of common stock."><strong>Basic EPS</strong></span></td><td>{format_value(summarized_data['Basic EPS'])}</td></tr>
        <tr><td><span data-tooltip="The total costs incurred by a company in its operations."><strong>Total Expenses</strong></span></td><td>{summarized_data['Total Expenses']}</td></tr>
        <tr><td><span data-tooltip="The difference between the revenue generated from a bank's assets and the expenses associated with paying its liabilities."><strong>Net Interest Income</strong></span></td><td>{summarized_data['Net Interest Income']}</td></tr>
        <tr><td><span data-tooltip="The cost incurred by a company for borrowed funds."><strong>Interest Expense</strong></span></td><td>{summarized_data['Interest Expense']}</td></tr>
        <tr><td><span data-tooltip="The amount earned by an entity's investments in interest-bearing assets."><strong>Interest Income</strong></span></td><td>{summarized_data['Interest Income']}</td></tr>
        <tr><td><span data-tooltip="The company's total earnings or profit."><strong>Net Income</strong></span></td><td>{summarized_data['Net Income']}</td></tr>
        <tr><td><span data-tooltip="A company's economic performance adjusted for unusual, non-recurring or one-time influences."><strong>Normalized Income</strong></span></td><td>{summarized_data['Normalized Income']}</td></tr>
            """
        financial_table += "</table>"
